<?php
  ...
  class FileLog extends BaseLog {
     ...
     */
    protected $_defaults = array( 'path' => LOGS, 'file' => null, 'types' => null, 'scopes' => array(), 'rotate' => 10, 'size' => 10485760, // 10MB
      'mask' => null, );
    ...
    public function write($type, $message) {
      ...
      if( ! $selfError && ! $exists && ! chmod($pathname, (int)$this->_config['mask']) ) {
        ...
        trigger_error(
          __d(
            'cake_dev', 'Could not apply permission mask "%s" on log file "%s"', array( $this->_config['mask'], $pathname )
          ), E_USER_WARNING
        );
        $selfError = false;
      }
