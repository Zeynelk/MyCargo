<?php
  ...
  class CakeLog {
     ...
     * @var array
     */
    protected static $_defaultLevels = array( 'emergency' => LOG_EMERG, 'alert' => LOG_ALERT, 'critical' => LOG_CRIT, 'error' => LOG_ERR, 'warning' => LOG_WARNING, 'notice' => LOG_NOTICE, 'info' => LOG_INFO, 'debug' => LOG_DEBUG, );
    ...
    public static function write($type, $message, $scope = array()) {
      ...
      foreach( self::$_Collection->enabled() as $streamName ) {
        ...
        if(// No config is a catch all (bc mode)
          (empty($types) && empty($scopes)) || // BC layer for mixing scope & level
          (in_array($type, $scopes)) || // no scopes, but has level
          (empty($scopes) && $correctLevel) || // exact scope + level
