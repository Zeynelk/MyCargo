  ...
  class TranslateBehavior extends ModelBehavior {
    ...
    protected function _addJoin(Model $Model, $query, $field, $aliasField, $locale) {
      ...
      if( is_array($locale) ) {
        ...
        $query['joins'][] = array( 'type' => 'INNER', 'alias' => $alias, 'table' => $joinTable, 'conditions' => array( "{$Model->alias}.{$Model->primaryKey}" => $db->identifier("{$alias}.foreign_key"), "{$alias}.model" => $Model->name, "{$alias}.{$RuntimeModel->displayField}" => $aliasField, "{$alias}.locale" => $locale ) );
      }
      return $query;
    }
    ...
    public function afterSave(Model $Model, $created, $options = array()) {
      ...
      foreach( $tempData as $field => $value ) {
        ...
        $translations = $RuntimeModel->find(
          'list', array( 'conditions' => $conditions, 'fields' => array( $RuntimeModel->alias . '.locale', $RuntimeModel->alias . '.id' ) )
        );
        foreach( $value as $_locale => $_value ) {
          ...
          if( array_key_exists($_locale, $translations) ) {
            $RuntimeModel->save(
              array( $RuntimeModel->alias => array_merge(
    ...
    public function bindTranslation(Model $Model, $fields, $reset = true) {
      ...
      foreach( $fields as $key => $value ) {
        ...
        if( $association === null ) {
          ...
          $associations[$association] = array_merge(
            $default, array( 'conditions' => array( 'model' => $Model->name, $RuntimeModel->displayField => $field ) )
          );
        }
      }
