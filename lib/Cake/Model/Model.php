<?php
  ...
  class Model extends Object implements CakeEventListener {
     ...
     * @var array
     */
    public $findMethods = array( 'all' => true, 'first' => true, 'count' => true, 'neighbors' => true, 'list' => true, 'threaded' => true );
    ...
    public function __construct($id = false, $table = null, $ds = null) {
      ...
      if( is_array($id) ) {
        extract(
          array_merge(
            array( 'id' => $this->id, 'table' => $this->useTable, 'ds' => $this->useDbConfig, 'name' => $this->name, 'alias' => $this->alias, 'plugin' => $this->plugin ), $id
          )
        );
      }
    ...
    public function implementedEvents() {
      return array( 'Model.beforeFind' => array( 'callable' => 'beforeFind', 'passParams' => true ), 'Model.afterFind' => array( 'callable' => 'afterFind', 'passParams' => true ), 'Model.beforeValidate' => array( 'callable' => 'beforeValidate', 'passParams' => true ), 'Model.afterValidate' => array( 'callable' => 'afterValidate' ), 'Model.beforeSave' => array( 'callable' => 'beforeSave', 'passParams' => true ), 'Model.afterSave' => array( 'callable' => 'afterSave', 'passParams' => true ), 'Model.beforeDelete' => array( 'callable' => 'beforeDelete', 'passParams' => true ), 'Model.afterDelete' => array( 'callable' => 'afterDelete' ), );
    }
    ...
    public function __isset($name) {
      ...
      if( ! ClassRegistry::isKeySet($className) && ! empty($dynamic) ) {
        $this->{$className} = new AppModel(
          array( 'name' => $className, 'table' => $this->hasAndBelongsToMany[$assocKey]['joinTable'], 'ds' => $this->useDbConfig )
        );
      } else {
      ...
      if( method_exists($db, 'listSources') ) {
        ...
        if( is_array($sources) && ! in_array(strtolower($this->tablePrefix . $tableName), array_map('strtolower', $sources)) ) {
          throw new MissingTableException(
            array( 'table' => $this->tablePrefix . $tableName, 'class' => $this->alias, 'ds' => $this->useDbConfig, )
          );
        }
    ...
    public function read($fields = null, $id = null) {
      ...
      if( $id !== null && $id !== false ) {
        $this->data = $this->find(
          'first', array( 'conditions' => array( $this->alias . '.' . $this->primaryKey => $id ), 'fields' => $fields )
        );
    ...
    protected function _saveMulti($joined, $id, $db) {
      foreach( $joined as $assoc => $data ) {
        ...
        if( $habtm['unique'] ) {
          ...
          $links = $Model->find(
            'all', array( 'conditions' => $conditions, 'recursive' => empty($habtm['conditions']) ? -1 : 0, 'fields' => $associationForeignKey, )
          );
    ...
    protected function _prepareUpdateFields($data) {
      ...
      $old = $this->find(
        'first', array( 'conditions' => array( $this->alias . '.' . $this->primaryKey => $this->id ), 'fields' => array_values($included), 'recursive' => -1 )
      );
    ...
    public function delete($id = null, $cascade = true) {
      ...
      if( ! empty($this->belongsTo) ) {
        foreach( $this->belongsTo as $assoc ) {
          ...
          $keys = $this->find(
            'first', array( 'fields' => $this->_collectForeignKeys(), 'conditions' => array( $this->alias . '.' . $this->primaryKey => $id ), 'recursive' => -1, 'callbacks' => false )
          );
          break;
        }
      }
    ...
    protected function _deleteDependent($id, $cascade) {
      ...
      foreach( array_merge($this->hasMany, $this->hasOne) as $assoc => $data ) {
        ...
        if( isset($data['exclusive']) && $data['exclusive'] ) {
          ...
          $records = $Model->find(
            'all', array( 'conditions' => $conditions, 'fields' => $Model->primaryKey )
          );
    ...
    protected function _deleteLinks($id) {
      foreach( $this->hasAndBelongsToMany as $data ) {
        ...
        $records = $Model->find(
          'all', array( 'conditions' => array( $Model->escapeField($data['foreignKey']) => $id ), 'fields' => $Model->primaryKey, 'recursive' => -1, 'callbacks' => false )
        );
    ...
    public function deleteAll($conditions, $cascade = true, $callbacks = false) {
      ...
      $ids = $this->find(
        'all', array_merge(
          array( 'fields' => "{$this->alias}.{$this->primaryKey}", 'order' => false, 'group' => "{$this->alias}.{$this->primaryKey}", 'recursive' => 0 ), compact('conditions')
        )
      );
    ...
    public function exists($id = null) {
      ...
      return (bool)$this->find(
        'count', array( 'conditions' => array( $this->alias . '.' . $this->primaryKey => $id ), 'recursive' => -1, 'callbacks' => false )
      );
    }
    ...
    public function buildQuery($type = 'first', $query = array()) {
      $query = array_merge(
        array( 'conditions' => null, 'fields' => null, 'joins' => array(), 'limit' => null, 'offset' => null, 'order' => null, 'page' => 1, 'group' => null, 'callbacks' => true, ), (array)$query
      );
      ...
      if( $state === 'before' ) {
        ...
        if( empty($query['fields']) ) {
          ...
          $query['fields'] = $db->calculate(
            $this, 'count', array( $db->expression($query['fields']), 'count' )
          );
        }
    ...
    protected function _clearCache($type = null) {
      ...
      $assoc = array( strtolower($pluralized), Inflector::underscore($pluralized) );
      foreach( $this->_associations as $association ) {
        foreach( $this->{$association} as $className ) {
          ...
          if( ! in_array(strtolower($pluralizedAssociation), $assoc) ) {
            $assoc = array_merge(
              $assoc, array( strtolower($pluralizedAssociation), Inflector::underscore($pluralizedAssociation) )
            );
          }
        }
      }
