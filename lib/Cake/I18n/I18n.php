        ...
        if( ($trans = $_this->_domains[$domain][$_this->_lang][$_this->category][$singular]) || ($plurals) && ($trans = $_this->_domains[$domain][$_this->_lang][$_this->category][$plural]) ) {
          if( is_array($trans) ) {
            if( isset($trans[$plurals]) ) {
              ...
              trigger_error(
                __d(
                  'cake_dev', 'Missing plural form translation for "%s" in "%s" domain, "%s" locale. ' . ' Check your po file for correct plurals and valid Plural-Forms header.', $singular, $domain, $_this->_lang
                ), E_USER_WARNING
              );
              $trans = $trans[0];
            }
