<?php
  ...
  class DbConfigTask extends AppShell {
     ...
     * @var array
     */
    protected $_defaultConfig = array( 'name' => 'default', 'datasource' => 'Database/Mysql', 'persistent' => 'false', 'host' => 'localhost', 'login' => 'root', 'password' => 'password', 'database' => 'project_name', 'schema' => null, 'prefix' => null, 'encoding' => null, 'port' => null );
    ...
    public function bake($configs) {
      ...
      if( file_exists($filename) ) {
        ...
        foreach( $temp as $configName => $info ) {
          ...
          $oldConfigs[] = array( 'name' => $configName, 'datasource' => $info['datasource'], 'persistent' => $info['persistent'], 'host' => $info['host'], 'port' => $info['port'], 'login' => $info['login'], 'password' => $info['password'], 'database' => $info['database'], 'prefix' => $info['prefix'], 'schema' => $info['schema'], 'encoding' => $info['encoding'] );
        }
      }
