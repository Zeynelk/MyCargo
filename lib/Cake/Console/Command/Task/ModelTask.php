<?php
  ...
  class ModelTask extends BakeTask {
    ...
    public function fieldValidation($fieldName, $metaData, $primaryKey = 'id') {
      ...
      $prompt = __d(
        'cake_console', "or enter in a valid regex validation string.\nAlternatively [s] skip the rest of the fields.\n"
      );
      ...
      while( $anotherValidator === 'y' ) {
        ...
        if( $this->interactive && $choice != $defaultChoice ) {
          $anotherValidator = $this->in(
            __d(
              'cake_console', "Would you like to add another validation rule\n" . "or skip the rest of the fields?"
            ), array( 'y', 'n', 's' ), 'n'
          );
    ...
    public function findBelongsTo(Model $model, $associations) {
      ...
      foreach( $fieldNames as $fieldName ) {
        ...
        if( $fieldName != $model->primaryKey && $fieldName !== 'parent_id' && $offset !== false ) {
          ...
          $associations['belongsTo'][] = array( 'alias' => $tmpModelName, 'className' => $tmpModelName, 'foreignKey' => $fieldName, );
        } elseif( $fieldName === 'parent_id' ) {
          $associations['belongsTo'][] = array( 'alias' => 'Parent' . $model->name, 'className' => $model->name, 'foreignKey' => $fieldName, );
        }
      }
      return $associations;
    }
      ...
      foreach( $this->_tables as $otherTable ) {
        ...
        foreach( $tempFieldNames as $fieldName ) {
          ...
          if( $fieldName !== $model->primaryKey && $fieldName === $foreignKey ) {
            $assoc = array( 'alias' => $tempOtherModel->name, 'className' => $tempOtherModel->name, 'foreignKey' => $fieldName );
          } elseif( $otherTable === $model->table && $fieldName === 'parent_id' ) {
            $assoc = array( 'alias' => 'Child' . $model->name, 'className' => $model->name, 'foreignKey' => $fieldName );
          }
          if( $assoc ) {
    ...
    public function findHasAndBelongsToMany(Model $model, $associations) {
      ...
      foreach( $this->_tables as $otherTable ) {
        ...
        if( $tableName && in_array($tableName, $this->_tables) ) {
          ...
          $associations['hasAndBelongsToMany'][] = array( 'alias' => $habtmName, 'className' => $habtmName, 'foreignKey' => $foreignKey, 'associationForeignKey' => $this->_modelKey($habtmName), 'joinTable' => $otherTable );
        }
      }
      return $associations;
    }
    ...
    public function doMoreAssociations(Model $model, $associations) {
      ...
      while( strtolower($wannaDoMoreAssoc) === 'y' ) {
        ...
        $this->out(
          __d(
            'cake_console', "For the following options be very careful to match your setup exactly.\n" . "Any spelling mistakes will cause errors."
          )
        );
        $this->hr();
    ...
    public function bake($name, $data = array()) {
      ...
      $defaults = array( 'associations' => array(), 'actsAs' => array(), 'validate' => array(), 'primaryKey' => 'id', 'useTable' => null, 'useDbConfig' => 'default', 'displayField' => null );
      ...
      $this->Template->set(
        array( 'plugin' => $this->plugin, 'pluginPath' => $pluginPath )
      );
    ...
    public function getName($useDbConfig = null) {
      ...
      while( ! $enteredModel ) {
        $enteredModel = $this->in(
          __d(
            'cake_console', "Enter a number from the list above,\n" . "type in the name of another model, or 'q' to exit"
          ), null, 'q'
        );
        ...
        if( ! $enteredModel || (int)$enteredModel > count($this->_modelNames) ) {
          $this->err(
            __d(
              'cake_console', "The model name you supplied was empty,\n" . "or the number you selected was not an option. Please try again."
            )
          );
          $enteredModel = '';
        }
    ...
    public function getOptionParser() {
      ...
      $parser->description(
        ...
        'name', array( 'help' => __d('cake_console', 'Name of the model to bake. Can use Plugin.name to bake plugin models.') )
      )->addSubcommand(
        'all', array( 'help' => __d('cake_console', 'Bake all model files with associations and validation.') )
      )->addOption(
        'plugin', array( 'short' => 'p', 'help' => __d('cake_console', 'Plugin to bake the model into.') )
      )->addOption(
        'theme', array( 'short' => 't', 'help' => __d('cake_console', 'Theme to use when baking code.') )
      )->addOption(
        'connection', array( 'short' => 'c', 'help' => __d('cake_console', 'The connection the model table is on.') )
      )->addOption(
        'force', array( 'short' => 'f', 'help' => __d('cake_console', 'Force overwriting existing files without prompting.') )
      )->epilog(
