    ...
    public function text($width = 72) {
      ...
      if( ! empty($subcommands) ) {
        ...
        foreach( $subcommands as $command ) {
          $out[] = String::wrap(
            $command->help($max), array( 'width' => $width, 'indent' => str_repeat(' ', $max), 'indentAt' => 1 )
          );
        }
        $out[] = '';
      ...
      if( ! empty($options) ) {
        ...
        foreach( $options as $option ) {
          $out[] = String::wrap(
            $option->help($max), array( 'width' => $width, 'indent' => str_repeat(' ', $max), 'indentAt' => 1 )
          );
        }
        $out[] = '';
      }
      ...
      if( ! empty($arguments) ) {
        ...
        foreach( $arguments as $argument ) {
          $out[] = String::wrap(
            $argument->help($max), array( 'width' => $width, 'indent' => str_repeat(' ', $max), 'indentAt' => 1 )
          );
        }
        $out[] = '';
      }
