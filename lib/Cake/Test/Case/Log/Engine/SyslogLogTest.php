<?php
  ...
  class SyslogLogTest extends CakeTestCase
  {
    ...
    public function testOpenLog()
    {
      ...
      $log = $this->getMock('SyslogLog', array('_open', '_write'));
      $log->config(
        array('prefix' => 'thing', 'flag' => LOG_NDELAY, 'facility' => LOG_MAIL, 'format' => '%s: %s'));
    ...
    {
      return array(array('emergency', LOG_EMERG), array('alert', LOG_ALERT), array('critical', LOG_CRIT), array('error', LOG_ERR), array('warning', LOG_WARNING), array('notice', LOG_NOTICE), array('info', LOG_INFO), array('debug', LOG_DEBUG));
    }
