<?php
  ...
  class XmlViewTest extends CakeTestCase {
    ...
    public function testRenderSerializeNoHelpers() {
      ...
      $Controller->set(
        array( '_serialize' => 'tags', 'tags' => array( 'cakephp', 'framework' ) )
      );
      $View = new XmlView($Controller);
      $View->render();
    ...
    public function testRenderWithoutViewMultiple() {
      ...
      $View = new XmlView($Controller);
      $output = $View->render(false);
      $expected = array( 'custom_name' => array( 'no' => $data['no'], 'user' => $data['user'] ) );
      $this->assertSame(Xml::build($expected)->asXML(), $output);
    }
    ...
    public function testRenderWithoutViewMultipleAndAlias() {
      ...
      $this->assertSame('application/xml', $Response->type());
      $output = $View->render(false);
      $expected = array( 'response' => array( 'new_name' => $data['original_name'], 'user' => $data['user'] ) );
      $this->assertSame(Xml::build($expected)->asXML(), $output);

      $Controller->set('_rootNode', 'custom_name');
      $View = new XmlView($Controller);
      $output = $View->render(false);
      $expected = array( 'custom_name' => array( 'new_name' => $data['original_name'], 'user' => $data['user'] ) );
      $this->assertSame(Xml::build($expected)->asXML(), $output);
    }
