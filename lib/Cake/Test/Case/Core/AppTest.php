<?php
  ...
  class AppTest extends CakeTestCase {
    ...
    public function testBuild() {
      ...
      $new = App::path('Model');
      $expected = array( APP . 'Model' . DS, '/path/to/models/' );
      $this->assertEquals($expected, $new);

      App::build();
      App::build(
        array( 'Model' => array( '/path/to/models/' ), 'Controller' => array( '/path/to/controllers/' ), ), App::APPEND
      );
      $new = App::path('Model');
      $expected = array( APP . 'Model' . DS, '/path/to/models/' );
      $this->assertEquals($expected, $new);
      $new = App::path('Controller');
      $expected = array( APP . 'Controller' . DS, '/path/to/controllers/' );
      $this->assertEquals($expected, $new);
    ...
    public function testCompatibleBuild() {
      ...
      $expected = array( '/path/to/datasources/', APP . 'Model' . DS . 'Datasource' . DS );
      ...
      $expected = array( '/path/to/behaviors/', APP . 'Model' . DS . 'Behavior' . DS );
      ...
      $expected = array( '/path/to/controllers/', APP . 'Controller' . DS );
      ...
      $expected = array( '/path/to/components/', APP . 'Controller' . DS . 'Component' . DS );
    ...
    public function testBuildPackage() {
      $pluginPaths = array( '/foo/bar', APP . 'Plugin' . DS, dirname(dirname(CAKE)) . DS . 'plugins' . DS );
      App::build(
        array( 'Plugin' => array( '/foo/bar' ) )
      );
      ...
      App::build(
        array( 'Service' => array( '%s' . 'Service' . DS, ), ), App::REGISTER
      );

      $expected = array( APP . 'Service' . DS, );
    ...
    public function testListObjects() {
      ...
      App::build(
        array( 'Model/Behavior' => App::core('Model/Behavior'), 'Controller' => App::core('Controller'), 'Controller/Component' => App::core('Controller/Component'), 'View' => App::core('View'), 'Model' => App::core('Model'), 'View/Helper' => App::core('View/Helper'), ), App::RESET
      );
      ...
      App::build(
        array( 'plugins' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Lib' . DS ) )
      );
    ...
    public function testListObjectsIgnoreDotDirectories() {
      ...
      App::build(
        array( 'plugins' => array( $path ) ), App::RESET
      );
      mkdir($path . '.svn');
      $result = App::objects('plugin', null, false);
      rmdir($path . '.svn');
    ...
    public function testListObjectsInPlugin() {
      App::build(
        array( 'Model' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Model' . DS ), 'plugins' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS ) ), App::RESET
      );
    ...
    public function testPluginImporting() {
      App::build(
        array( 'Lib' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Lib' . DS ), 'Plugin' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS ) )
      );
      CakePlugin::load(array( 'TestPlugin', 'TestPluginTwo' ));
    ...
    public function testImportingHelpersFromAlternatePaths() {
      ...
      App::build(
        array( 'View/Helper' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'View' . DS . 'Helper' . DS ) )
      );
      $this->assertFalse(class_exists('BananaHelper', false), 'BananaHelper exists, cannot test importing it.');
    ...
    public function testFileLoadingWithArray() {
      $type = array( 'type' => 'File', 'name' => 'SomeName', 'parent' => false, 'file' => CAKE . DS . 'Config' . DS . 'config.php' );
      $file = App::import($type);
      $this->assertTrue($file);

      $type = array( 'type' => 'File', 'name' => 'NoFile', 'parent' => false, 'file' => CAKE . 'Config' . DS . 'cake' . DS . 'config.php' );
      $file = App::import($type);
      $this->assertFalse($file);
    }
    ...
    public function testFileLoadingReturnValue() {
      ...
      $type = array( 'type' => 'File', 'name' => 'OtherName', 'parent' => false, 'file' => CAKE . 'Config' . DS . 'config.php', 'return' => true );
      $file = App::import($type);
      $this->assertTrue(! empty($file));
    ...
    public function testLoadingWithSearchArray() {
      $type = array( 'type' => 'File', 'name' => 'RandomName', 'parent' => false, 'file' => 'config.php', 'search' => array( CAKE . 'Config' . DS ) );
      $file = App::import($type);
      $this->assertTrue($file);

      $type = array( 'type' => 'File', 'name' => 'AnotherRandomName', 'parent' => false, 'file' => 'config.php', 'search' => array( CAKE ) );
      $file = App::import($type);
      $this->assertFalse($file);
    }
    ...
    public function testMultipleLoading() {
      ...
      App::build(
        array( 'Model' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Model' . DS ) )
      );
    ...
    public function testLoadingVendor() {
      App::build(
        array( 'plugins' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS ), 'vendors' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Vendor' . DS ), ), App::RESET
      );
      CakePlugin::load(array( 'TestPlugin', 'TestPluginTwo' ));
    ...
    public function testLoadClassInLibs() {
      App::build(
        array( 'libs' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Lib' . DS ), 'plugins' => array( CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS ) ), App::RESET
      );
      CakePlugin::load(array( 'TestPlugin', 'TestPluginTwo' ));
