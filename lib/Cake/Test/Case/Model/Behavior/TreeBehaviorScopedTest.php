<?php
  ...
  class TreeBehaviorScopedTest extends CakeTestCase
  {
     ...
     * @var array
     */
    public $settings = array('modelClass' => 'FlagTree', 'leftField' => 'lft', 'rightField' => 'rght', 'parentField' => 'parent_id');
    ...
    public function testStringScope()
    {
      ...
      $expected = array(array('FlagTree' => array('id' => '3', 'name' => '1.1.1', 'parent_id' => '2', 'lft' => '3', 'rght' => '4', 'flag' => '0')), array('FlagTree' => array('id' => '4', 'name' => '1.1.2', 'parent_id' => '2', 'lft' => '5', 'rght' => '6', 'flag' => '0')), array('FlagTree' => array('id' => '5', 'name' => '1.1.3', 'parent_id' => '2', 'lft' => '7', 'rght' => '8', 'flag' => '0')));
    ...
    public function testSaveWithParentAndInvalidScope()
    {
      ...
      $this->Tree->Behaviors->load(
        'Tree', array('scope' => array('FlagTree.flag' => 100)));
    ...
    public function testTranslatingTree()
    {
      ...
      $data = array('FlagTree' => array('title' => 'name #1', 'name' => 'test', 'locale' => 'eng', 'parent_id' => null,));
      $this->Tree->save($data);
      $result = $this->Tree->find('all');
      $expected = array(array('FlagTree' => array('id' => 1, 'title' => 'name #1', 'name' => 'test', 'parent_id' => null, 'lft' => 1, 'rght' => 2, 'flag' => 0, 'locale' => 'eng',)));
      $this->assertEquals($expected, $result);
      ...
      $this->Tree->save($data);
      $result = $this->Tree->find('all');
      $expected = array(array('FlagTree' => array('id' => 1, 'title' => 'Named 2', 'name' => 'test', 'parent_id' => null, 'lft' => 1, 'rght' => 2, 'flag' => 0, 'locale' => 'eng',)));
      $this->assertEquals($expected, $result);
      ...
      $this->Tree->id = 1;
      $data = array('FlagTree' => array('id' => 1, 'parent_id' => null, 'title' => 'namen #1', 'name' => 'test', 'locale' => 'deu',));
      $this->Tree->save($data);

      $this->Tree->locale = 'deu';
      $result = $this->Tree->find('all');
      $expected = array(array('FlagTree' => array('id' => 1, 'title' => 'namen #1', 'name' => 'test', 'parent_id' => null, 'lft' => 1, 'rght' => 2, 'flag' => 0, 'locale' => 'deu',)));
      $this->assertEquals($expected, $result);

      // Save with bindTranslation
      $this->Tree->locale = 'eng';
      $data = array('title' => array('eng' => 'New title', 'spa' => 'Nuevo leyenda'), 'name' => 'test', 'parent_id' => null);
      ...
      $expected = array('FlagTree' => array('id' => 2, 'parent_id' => null, 'locale' => 'eng', 'name' => 'test', 'title' => 'New title', 'flag' => 0, 'lft' => 3, 'rght' => 4), 'Title' => array(array('id' => 21, 'locale' => 'eng', 'model' => 'FlagTree', 'foreign_key' => 2, 'field' => 'title', 'content' => 'New title'), array('id' => 22, 'locale' => 'spa', 'model' => 'FlagTree', 'foreign_key' => 2, 'field' => 'title', 'content' => 'Nuevo leyenda')),);
      $this->assertEquals($expected, $result);
    }
    ...
    public function testAliasesWithScopeInTwoTreeAssociations()
    {
      ...
      $this->Tree->bindModel(
        array('hasMany' => array('SecondTree' => array('className' => 'NumberTreeTwo', 'foreignKey' => 'number_tree_id'))));
      $this->TreeTwo->bindModel(
        array('belongsTo' => array('FirstTree' => array('className' => $modelClass, 'foreignKey' => 'number_tree_id'))));
      $this->TreeTwo->Behaviors->load(
        'Tree', array('scope' => 'FirstTree'));

      $data = array('NumberTreeTwo' => array('name' => 'First', 'number_tree_id' => $record['FlagTree']['id']));
      $this->TreeTwo->create();
      $result = $this->TreeTwo->save($data);
      $this->assertFalse(empty($result));

      $result = $this->TreeTwo->find('first');
      $expected = array('NumberTreeTwo' => array('id' => 1, 'name' => 'First', 'number_tree_id' => $record['FlagTree']['id'], 'parent_id' => null, 'lft' => 1, 'rght' => 2));
      $this->assertEquals($expected, $result);
    }
    ...
    public function testGenerateTreeListWithScope()
    {
      ...
      $expected = array(1 => '1. Root', 2 => '_1.1');
      $this->assertEquals($expected, $result);
    ...
    public function testRecoverUsingParentMode()
    {
      ...
      $result = $this->Tree->find(
        'first', array('fields' => array('name', $parentField, $leftField, $rightField, 'flag'), 'conditions' => array('name' => 'Main'), 'recursive' => -1));
      $expected = array($modelClass => array('name' => 'Main', $parentField => null, $leftField => 1, $rightField => 10, 'flag' => 1));
      $this->assertEquals($expected, $result);
    }
