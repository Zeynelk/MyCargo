<?php
  ...
  class ModelDeleteTest extends BaseModelTest {
    ...
    public function testDeleteHabtmReferenceWithConditions() {
      ...
      $result = $Portfolio->find(
        'first', array( 'conditions' => array( 'Portfolio.id' => 1 ) )
      );
      ...
      $result = $Portfolio->ItemsPortfolio->find(
        'all', array( 'conditions' => array( 'ItemsPortfolio.portfolio_id' => 1 ) )
      );
    ...
    public function testDeleteDependentWithConditions() {
      ...
      $result = $OverallFavorite->find(
        'all', array( 'fields' => array( 'model_type', 'model_id', 'priority' ) )
      );
    ...
    public function testDelete() {
      ...
      $result = $TestModel->find(
        'all', array( 'fields' => array( 'id', 'title' ) )
      );
      ...
      $result = $Uuid->find(
        'all', array( 'conditions' => array( 'id' => $data ), 'fields' => array( 'id' ), 'order' => 'id' )
      );
    ...
    public function testDeleteAll() {
      ...
      $TestModel->recursive = -1;
      $result = $TestModel->find(
        'all', array( 'fields' => array( 'id', 'user_id', 'title', 'published' ), 'order' => array( 'Article.id' => 'ASC' ) )
      );
    ...
    public function testDeleteAllMultipleRowsPerId() {
      ...
      $TestModel->unbindModel(
        array( 'belongsTo' => array( 'User' ), 'hasMany' => array( 'Comment' ), 'hasAndBelongsToMany' => array( 'Tag' ) ), false
      );
      $TestModel->bindModel(
        array( 'belongsTo' => array( 'User' => array( 'foreignKey' => false, 'conditions' => array( 'Article.user_id = 1' ) ) ) ), false
      );
    ...
    public function testDeleteLinksWithPLuginJoinModel() {
      ...
      $Article->bindModel(
        array( 'hasAndBelongsToMany' => array( 'Tag' => array( 'with' => 'TestPlugin.ArticlesTag' ) ) ), false
      );
    ...
    public function testDeleteDependent() {
      $this->loadFixtures(
        'Bidding', 'BiddingMessage', 'Article', 'ArticlesTag', 'Comment', 'User', 'Attachment'
      );
      $Bidding = new Bidding();
      ...
      $Article = new Article();
      $result = $Article->Comment->find(
        'count', array( 'conditions' => array( 'Comment.article_id' => 1 ) )
      );
      $this->assertEquals(4, $result);
    ...
    public function testDeleteLinksWithMultipleHabtmAssociations() {
      ...
      $JoinA->JoinAsJoinC->create(
        array( 'join_a_id' => 1, 'join_c_id' => 2, )
      );
      $JoinA->JoinAsJoinC->save();
      $JoinA->JoinAsJoinB->create(
        array( 'join_a_id' => 1, 'join_b_id' => 2, )
      );
      ...
      $joinedBs = $JoinA->JoinAsJoinB->find(
        'count', array( 'conditions' => array( 'JoinAsJoinB.join_a_id' => 1 ) )
      );
      $this->assertEquals(0, $joinedBs, 'JoinA/JoinB link records left over. %s');

      $joinedBs = $JoinA->JoinAsJoinC->find(
        'count', array( 'conditions' => array( 'JoinAsJoinC.join_a_id' => 1 ) )
      );
    ...
    public function testDeleteHabtmPostgresFailure() {
      ...
      $Tag->bindModel(
        array( 'hasAndBelongsToMany' => array( 'Article' => array( 'className' => 'Article', 'unique' => true ) ) ), true
      );

      // Article 1 should have Tag.1 and Tag.2
      $before = $Article->find(
        "all", array( "conditions" => array( "Article.id" => 1 ), )
      );
      ...
      $submittedData = array( "Tag" => array( "id" => 1, 'tag' => 'tag1' ), "Article" => array( "Article" => array( 1 ) ) );
      $Tag->save($submittedData);

      // One more submission (The other way around) to make sure the reverse save looks good.
      $submittedData = array( "Article" => array( "id" => 2, 'title' => 'second article' ), "Tag" => array( "Tag" => array( 2, 3 ) ) );
    ...
    public function testBeforeDeleteWipingTable() {
      ...
      $Comment->create(
        array( 'article_id' => 1, 'user_id' => 2, 'comment' => 'new record', 'published' => 'Y' )
      );
      $Comment->save();
