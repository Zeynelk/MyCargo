<?php
  ...
  class ModelReadTest extends BaseModelTest {
    ...
    public function testFetchingNonUniqueFKJoinTableRecords() {
      ...
      $joinThingData = array( 'JoinThing' => array( 'something_id' => 1, 'something_else_id' => 2, 'doomed' => '0', 'created' => '2007-03-18 10:39:23', 'updated' => '2007-03-18 10:41:31' ) );
    ...
    public function testGroupBy() {
      ...
      $result = $Thread->find(
        'all', array( 'group' => 'Thread.project_id', 'order' => 'Thread.id ASC' )
      );
      ...
      $rows = $Thread->find(
        'all', array( 'group' => 'Thread.project_id', 'fields' => array( 'Thread.project_id', 'COUNT(*) AS total' ) )
      );
      $result = array();
      foreach( $rows as $row ) {
      ...
      $rows = $Thread->find(
        'all', array( 'group' => 'Thread.project_id', 'fields' => array( 'Thread.project_id', 'COUNT(*) AS total' ), 'order' => 'Thread.project_id' )
      );
      $result = array();
      foreach( $rows as $row ) {
        $result[$row['Thread']['project_id']] = $row[0]['total'];
      ...
      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => 'Thread.project_id' )
      );
      ...
      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => 'Thread.project_id, Project.id' )
      );
      $this->assertEquals($expected, $result);

      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => 'project_id' )
      );
      $this->assertEquals($expected, $result);

      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => array( 'project_id' ) )
      );
      $this->assertEquals($expected, $result);

      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => array( 'project_id', 'Project.id' ) )
      );
      $this->assertEquals($expected, $result);

      $result = $Thread->find(
        'all', array( 'conditions' => array( 'Thread.project_id' => 1 ), 'group' => array( 'Thread.project_id', 'Project.id' ) )
      );
      $this->assertEquals($expected, $result);
      ...
      $result = $Product->find(
        'all', array( 'fields' => array( 'Product.type', 'MIN(Product.price) as price' ), 'group' => 'Product.type', 'order' => 'Product.type ASC' )
      );
      $this->assertEquals($expected, $result);

      $result = $Product->find(
        'all', array( 'fields' => array( 'Product.type', 'MIN(Product.price) as price' ), 'group' => array( 'Product.type' ), 'order' => 'Product.type ASC' )
      );
      $this->assertEquals($expected, $result);
    }
    ...
    public function testPreparedQuery() {
      ...
      $expected = array( '0' => array( $this->db->fullTableName('articles', false, false) => array( 'title' => 'First Article', 'published' => 'Y' ) ) );
      ...
      $this->assertTrue(
        isset($result[0][$this->db->fullTableName('articles', false, false)]) || isset($result[0][0])
      );
      $result = $this->db->getQueryCache($query, $params);
      ...
      $this->assertTrue(is_array($result));
      $this->assertTrue(
        isset($result[0][$this->db->fullTableName('articles', false, false)]['title']) || isset($result[0][0]['title'])
      );
    ...
    public function testAfterFindUnset() {
      ...
      $model->bindModel(
        array( 'hasMany' => array( 'ModifiedComment' => array( 'className' => 'ModifiedComment', 'foreignKey' => 'article_id', ) ) )
      );
    ...
    public function testFindAllThreaded() {
      ...
      $result = $TestModel->find(
        'threaded', array( 'conditions' => array( 'Category.name LIKE' => 'Category 1%' ) )
      );
      ...
      $result = $TestModel->find(
        'threaded', array( 'fields' => 'id, parent_id, name' )
      );
      ...
      $result = $TestModel->find(
        'threaded', array( 'conditions' => array( 'Category.name LIKE' => 'Category 3%' ) )
      );
      ...
      $result = $TestModel->find(
        'threaded', array( 'conditions' => array( 'Category.name LIKE' => 'Category 1.1%' ) )
      );
      $expected = array( array( 'Category' => array( 'id' => '2', 'parent_id' => '1', 'name' => 'Category 1.1', 'created' => '2007-03-18 15:30:23', 'updated' => '2007-03-18 15:32:31' ), 'children' => array( array( 'Category' => array( 'id' => '7', 'parent_id' => '2', 'name' => 'Category 1.1.1', 'created' => '2007-03-18 15:30:23', 'updated' => '2007-03-18 15:32:31' ), 'children' => array() ), array( 'Category' => array( 'id' => '8', 'parent_id' => '2', 'name' => 'Category 1.1.2', 'created' => '2007-03-18 15:30:23', 'updated' => '2007-03-18 15:32:31' ), 'children' => array() ) ) ) );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'threaded', array( 'fields' => 'id, parent_id, name', 'conditions' => array( 'Category.id !=' => 2 ) )
      );
      $expected = array( array( 'Category' => array( 'id' => '1', 'parent_id' => '0', 'name' => 'Category 1' ), 'children' => array( array( 'Category' => array( 'id' => '3', 'parent_id' => '1', 'name' => 'Category 1.2' ), 'children' => array() ) ) ), array( 'Category' => array( 'id' => '4', 'parent_id' => '0', 'name' => 'Category 2' ), 'children' => array() ), array( 'Category' => array( 'id' => '5', 'parent_id' => '0', 'name' => 'Category 3' ), 'children' => array( array( 'Category' => array( 'id' => '6', 'parent_id' => '5', 'name' => 'Category 3.1' ), 'children' => array() ) ) ) );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'all', array( 'fields' => 'id, name, parent_id', 'conditions' => array( 'Category.id !=' => 1 ) )
      );
      ...
      $result = $TestModel->find(
        'threaded', array( 'fields' => 'id, parent_id, name', 'conditions' => array( 'Category.id !=' => 1 ) )
      );
    ...
    public function testFindNeighbors() {
      ...
      $result = $TestModel->find(
        'neighbors', array( 'fields' => array( 'id' ) )
      );
      ...
      $expected = array( 'prev' => null, 'next' => array( 'Article' => array( 'id' => 2, 'user_id' => 3, 'title' => 'Second Article', 'body' => 'Second Article Body', 'published' => 'Y', 'created' => '2007-03-18 10:41:23', 'updated' => '2007-03-18 10:43:31' ) ) );
      $this->assertEquals($expected, $result);
      ...
      $TestModel->id = 3;
      $result = $TestModel->find('neighbors', array( 'recursive' => -1 ));
      $expected = array( 'prev' => array( 'Article' => array( 'id' => 2, 'user_id' => 3, 'title' => 'Second Article', 'body' => 'Second Article Body', 'published' => 'Y', 'created' => '2007-03-18 10:41:23', 'updated' => '2007-03-18 10:43:31' ) ), 'next' => null );
      $this->assertEquals($expected, $result);
    ...
    public function testFindNeighborsNoPrev() {
      ...
      $result = $Article->find(
        'neighbors', array( 'field' => 'Article.title', 'value' => 'Second Article', 'fields' => array( 'id' ), 'conditions' => array( 'Article.title LIKE' => '%Article%' ), 'recursive' => 0, )
      );
      $expected = array( 'prev' => null, 'next' => null );
      $this->assertEquals($expected, $result);
    }
    ...
    public function testBindUnbind() {
      ...
      $result = $TestModel->find(
        'all', array( 'fields' => 'User.id, User.user', 'order' => array( 'User.id' => 'ASC' ), )
      );
      ...
      $result = $TestModel->bindModel(
        array( 'hasMany' => array( 'Comment' => array( 'className' => 'Comment', 'conditions' => 'Comment.published = \'Y\'' ) ) )
      );
      $this->assertTrue($result);
      ...
      $TestModel2->bindModel(
        array( 'belongsTo' => array( 'FeatureSet' => array( 'className' => 'FeatureSet', 'conditions' => array( 'active' => true ) ) ) )
      );
      $expected['conditions'] = array( 'active' => true );
      $this->assertEquals($expected, $TestModel2->belongsTo['FeatureSet']);

      $TestModel2->bindModel(
        array( 'belongsTo' => array( 'FeatureSet' => array( 'className' => 'FeatureSet', 'foreignKey' => false, 'conditions' => array( 'Feature.name' => 'DeviceType.name' ) ) ) )
      );
      ...
      $TestModel2->bindModel(
        array( 'hasMany' => array( 'NewFeatureSet' => array( 'className' => 'FeatureSet', 'conditions' => array( 'active' => true ) ) ) )
      );
    ...
    public function testBindMultipleTimes() {
      ...
      $result = $TestModel->bindModel(
        array( 'hasMany' => array( 'Items' => array( 'className' => 'Comment' ) ) )
      );
      $this->assertTrue($result);

      $result = $TestModel->find(
        'all', array( 'fields' => 'User.id, User.user' )
      );
      ...
      $result = $TestModel->bindModel(
        array( 'hasMany' => array( 'Items' => array( 'className' => 'Article' ) ) )
      );
      $this->assertTrue($result);
    ...
    public function testBindWithCustomPrimaryKey() {
      ...
      $Model->bindModel(
        array( 'belongsTo' => array( 'Tag' => array( 'className' => 'Tag', 'foreignKey' => 'story' ) ) )
      );
    ...
    public function testAssociationAfterFind() {
      ...
      $TestModel = new Post();
      $result = $TestModel->find(
        'all', array( 'order' => array( 'Post.id' => 'ASC' ) )
      );
      ...
      $Author->Post->bindModel(
        array( 'hasMany' => array( 'Comment' => array( 'className' => 'ModifiedComment', 'foreignKey' => 'article_id', ) ) )
      );
      $result = $Author->find(
        'all', array( 'conditions' => array( 'Author.id' => 1 ), 'order' => array( 'Author.id' => 'ASC' ), 'recursive' => 2 )
      );
      $expected = array( 'id' => 1, 'article_id' => 1, 'user_id' => 2, 'comment' => 'First Comment for First Article', 'published' => 'Y', 'created' => '2007-03-18 10:45:23', 'updated' => '2007-03-18 10:47:31', 'callback' => 'Fire' );
    ...
    public function testDeeperAssociationAfterFind() {
      ...
      $Post->Comment->bindModel(
        array( 'hasOne' => array( 'Attachment' => array( 'className' => 'ModifiedAttachment', ) ) )
      );

      $result = $Post->find(
        'first', array( 'conditions' => array( 'Post.id' => 2 ), 'recursive' => 2 )
      );
    ...
    public function testAssociationAfterFindCalbacksDisabled() {
      ...
      $TestModel = new Post();
      $result = $TestModel->find(
        'all', array( 'callbacks' => false, 'order' => array( 'Post.id' => 'ASC' ), )
      );
      ...
      $result = $Author->find(
        'all', array( 'conditions' => array( 'Author.id' => 1 ), 'recursive' => 2, 'order' => array( 'Author.id' => 'ASC' ), 'callbacks' => false )
      );
    ...
    public function testHabtmFinderQuery() {
      ...
      $sql = $this->db->buildStatement(
        array( 'fields' => $this->db->fields(
          $Article->Tag, null, array( 'Tag.id', 'Tag.tag', 'ArticlesTag.article_id', 'ArticlesTag.tag_id' )
        ), 'table' => $this->db->fullTableName('tags'), 'alias' => 'Tag', 'limit' => null, 'offset' => null, 'group' => null, 'joins' => array( array( 'alias' => 'ArticlesTag', 'table' => 'articles_tags', 'conditions' => array( array( "ArticlesTag.article_id" => '{$__cakeID__$}' ), array( "ArticlesTag.tag_id" => $this->db->identifier('Tag.id') ) ) ) ), 'conditions' => array(), 'order' => null ), $Article
      );
    ...
    public function testFindAll() {
      ...
      $expected = array( array( 'User' => array( 'id' => '3', 'user' => 'larry', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:20:23', 'updated' => '2007-03-17 01:22:31' ) ), array( 'User' => array( 'id' => '4', 'user' => 'garrett', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:22:23', 'updated' => '2007-03-17 01:24:31' ) ) );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'User.id !=' => '0', 'User.user LIKE' => '%arr%' ) )
      );
      $expected = array( array( 'User' => array( 'id' => '3', 'user' => 'larry', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:20:23', 'updated' => '2007-03-17 01:22:31' ) ), array( 'User' => array( 'id' => '4', 'user' => 'garrett', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:22:23', 'updated' => '2007-03-17 01:24:31' ) ) );
      $this->assertEquals($expected, $result);
      ...
      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'or' => array( 'User.id' => '0', 'User.user LIKE' => '%a%' ) ) )
      );
      ...
      $expected = array( array( 'User' => array( 'user' => 'garrett' ) ), array( 'User' => array( 'user' => 'larry' ) ), array( 'User' => array( 'user' => 'mariano' ) ), array( 'User' => array( 'user' => 'nate' ) ) );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find('all', array( 'fields' => 'User.user', 'order' => 'User.user DESC' ));
      $expected = array( array( 'User' => array( 'user' => 'nate' ) ), array( 'User' => array( 'user' => 'mariano' ) ), array( 'User' => array( 'user' => 'larry' ) ), array( 'User' => array( 'user' => 'garrett' ) ) );
      $this->assertEquals($expected, $result);
      ...
      $ids = array( 4 => 1, 5 => 3 );
      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'User.id' => $ids ), 'order' => 'User.id' )
      );
      $expected = array( array( 'User' => array( 'id' => '1', 'user' => 'mariano', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:16:23', 'updated' => '2007-03-17 01:18:31' ) ), array( 'User' => array( 'id' => '3', 'user' => 'larry', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:20:23', 'updated' => '2007-03-17 01:22:31' ) ) );
      $this->assertEquals($expected, $result);
      ...
      if( ! ($this->db instanceof Sqlserver) ) {
        ...
        $expected = array( array( 'User' => array( 'id' => '4', 'user' => 'garrett', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:22:23', 'updated' => '2007-03-17 01:24:31' ) ) );
        $this->assertEquals($expected, $result);
    ...
    public function testFindAllArrayConditions() {
      ...
      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'User.id' => array( 3 ) ), )
      );
      $expected = array( array( 'User' => array( 'id' => '3', 'user' => 'larry', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99', 'created' => '2007-03-17 01:20:23', 'updated' => '2007-03-17 01:22:31' ) ) );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'User.user' => array( 'larry' ) ), )
      );
      $this->assertEquals($expected, $result);
    }
    ...
    public function testFindList() {
      ...
      $result = $TestModel->find(
        'list', array( 'order' => 'Article.title ASC' )
      );
      ...
      if( $db instanceof Mysql ) {
        $result = $TestModel->find(
          'list', array( 'order' => array( 'FIELD(Article.id, 3, 2) ASC', 'Article.title ASC' ) )
        );
      ...
      $result = Hash::combine(
        $TestModel->find(
          'all', array( 'order' => 'Article.title ASC', 'fields' => array( 'id', 'title' ) )
      ...
      $result = Hash::combine(
        $TestModel->find(
          'all', array( 'order' => 'Article.title ASC' )
      ...
      $result = Hash::combine(
        $TestModel->find(
          'all', array( 'order' => 'Article.title ASC', 'fields' => array( 'id', 'title', 'user_id' ) )
      ...
      $TestModel = new Post();
      $result = $TestModel->find(
        'list', array( 'fields' => 'Post.title' )
      );
      $expected = array( 1 => 'First Post', 2 => 'Second Post', 3 => 'Third Post' );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'list', array( 'fields' => 'title' )
      );
      $expected = array( 1 => 'First Post', 2 => 'Second Post', 3 => 'Third Post' );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'list', array( 'fields' => array( 'title', 'id' ) )
      );
      $expected = array( 'First Post' => '1', 'Second Post' => '2', 'Third Post' => '3' );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'list', array( 'fields' => array( 'title', 'id', 'created' ) )
      );
      ...
      $TestModel = new User();
      $result = $TestModel->find(
        'list', array( 'fields' => array( 'User.user', 'User.password' ) )
      );
      ...
      $TestModel = new ModifiedAuthor();
      $result = $TestModel->find(
        'list', array( 'fields' => array( 'Author.id', 'Author.user' ) )
      );
      ...
      $TestModel->displayField = 'title';
      $result = $TestModel->find(
        'list', array( 'conditions' => array( 'User.user' => 'mariano' ), 'recursive' => 0 )
      );
    ...
    public function testFindListArrayCondition() {
      ...
      $result = $TestModel->find(
        'list', array( 'fields' => array( 'id', 'user' ), 'conditions' => array( 'User.id' => array( 3 ) ), )
      );
      $expected = array( 3 => 'larry' );
      $this->assertEquals($expected, $result);

      $result = $TestModel->find(
        'list', array( 'fields' => array( 'id', 'user' ), 'conditions' => array( 'User.user' => array( 'larry' ) ), )
      );
      $this->assertEquals($expected, $result);
    }
    ...
    public function testFindUnique() {
      ...
      $this->assertFalse(
        $TestModel->isUnique(
          array( 'user' => 'nate', 'password' => '5f4dcc3b5aa765d61d8327deb882cf99' )
        )
      );
    }
    ...
    public function testFindCount() {
      ...
      $expected = count(
        $Article->find(
          'all', array( 'fields' => array( 'Article.user_id' ), 'group' => 'Article.user_id' )
        )
      );
      ...
      $expected = count(
        $Article->find(
          'all', array( 'fields' => array( 'Article.user_id' ), 'conditions' => array( 'Article.user_id' => 1 ), 'group' => 'Article.user_id' )
        )
      );
      $result = $Article->find(
        'count', array( 'conditions' => array( 'Article.user_id' => 1 ), 'group' => array( 'Article.user_id' ), )
      );
      $this->assertEquals($expected, $result);
    }
    ...
    public function testFindCountWithDbExpressions() {
      ...
      $result = $TestModel->find(
        'count', array( 'conditions' => array( $db->expression('Project.name = \'Project 3\'') ) )
      );
      $this->assertEquals(1, $result);

      $result = $TestModel->find(
        'count', array( 'conditions' => array( 'Project.name' => $db->expression('\'Project 3\'') ) )
      );
      $this->assertEquals(1, $result);
    }
    ...
    public function testRecursiveFindAll() {
      ...
      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'Article.user_id' => 3 ), 'limit' => 1, 'recursive' => 2 )
      );
      ...
      $Featured->bindModel(
        array( 'belongsTo' => array( 'ArticleFeatured' => array( 'conditions' => "ArticleFeatured.published = 'Y'", 'fields' => 'id, title, user_id, published' ) ) )
      );

      $Featured->ArticleFeatured->unbindModel(
        array( 'hasMany' => array( 'Attachment', 'Comment' ), 'hasAndBelongsToMany' => array( 'Tag' ) )
      );

      $orderBy = 'ArticleFeatured.id ASC';
      $result = $Featured->find(
        'all', array( 'order' => $orderBy, 'limit' => 3 )
      );
    ...
    public function testRecursiveFindAllWithLimit() {
      ...
      $result = $TestModel->find(
        'all', array( 'conditions' => array( 'Article.user_id' => 1 ) )
      );
    ...
    public function testVirtualFields() {
      ...
      $result = $Post->find(
        'first', array( 'conditions' => array( 'two' => 2 ), 'limit' => 1 )
      );
      $this->assertEquals(2, $result['Post']['two']);

      $result = $Post->find(
        'first', array( 'conditions' => array( 'two <' => 3 ), 'limit' => 1 )
      );
      $this->assertEquals(2, $result['Post']['two']);

      $result = $Post->find(
        'first', array( 'conditions' => array( 'NOT' => array( 'two >' => 3 ) ), 'limit' => 1 )
      );
      $this->assertEquals(2, $result['Post']['two']);
      ...
      $result = $Post->find(
        'first', array( 'conditions' => array( 'other_field' => 3 ), 'limit' => 1 )
      );
      ...
      $Post->virtualFields = array( 'other_field' => 'Post.id + 1' );
      $result = $Post->find(
        'all', array( 'fields' => array( $dbo->calculate($Post, 'max', array( 'other_field' )) ) )
      );
    ...
    public function testVirtualFieldsOrder() {
      ...
      $expected = array( '3' => 'Third Post', '2' => 'Second Post', '1' => 'First Post' );
      $this->assertEquals($expected, $result);
    ...
    public function testVirtualFieldsMysql() {
      ...
      $Post->create();
      $Post->virtualFields = array( 'low_title' => 'lower(Post.title)', 'unique_test_field' => 'COUNT(Post.id)' );

      $expectation = array( 'Post' => array( 'low_title' => 'first post', 'unique_test_field' => 1 ) );

      $result = $Post->find(
        'first', array( 'fields' => array_keys($Post->virtualFields), 'group' => array( 'low_title' ) )
      );

      $this->assertEquals($expectation, $result);

      $Author = ClassRegistry::init('Author');
      $Author->virtualFields = array( 'full_name' => 'CONCAT(Author.user, " ", Author.id)' );

      $result = $Author->find(
        'first', array( 'conditions' => array( 'Author.user' => 'mariano' ), 'fields' => array( 'Author.password', 'Author.full_name' ), 'recursive' => -1 )
      );
      $this->assertTrue(isset($result['Author']['full_name']));

      $result = $Author->find(
        'first', array( 'conditions' => array( 'Author.user' => 'mariano' ), 'fields' => array( 'Author.full_name', 'Author.password' ), 'recursive' => -1 )
      );
      $this->assertTrue(isset($result['Author']['full_name']));
    }
    ...
    public function testVirtualFieldAsAString() {
      $this->loadFixtures('Post', 'Author');
      $Post = new Post();
      $Post->virtualFields = array( 'writer' => 'Author.user' );
      $result = $Post->find('first');
    ...
    public function testNotInArrayWithOneValue() {
      ...
      $result = $Article->find(
        'all', array( 'conditions' => array( 'Article.id NOT' => array( 1 ) ) )
      );
    ...
    public function testNotEqualsInArrayWithOneValue() {
      ...
      $result = $Article->find(
        'all', array( 'conditions' => array( 'Article.id !=' => array( 1 ) ) )
      );
      $this->assertTrue(is_array($result) && ! empty($result));
    }
    ...
    public function testRelatedAfterFindCallback() {
      ...
      $Something->bindModel(
        array( 'hasMany' => array( 'HasMany' => array( 'className' => 'JoinThing', 'foreignKey' => 'something_id' ) ), 'hasOne' => array( 'HasOne' => array( 'className' => 'JoinThing', 'foreignKey' => 'something_id' ) ) )
      );
      ...
      $JoinThing->unbindModel(
        array( 'belongsTo' => array( 'Something' ) )
      );
