<?php
  ...
  class FileEngineTest extends CakeTestCase
  {
    ...
    public function testClearWithPrefixes()
    {
      ...
      $FileOne->init(
        array('prefix' => 'prefix_one_', 'duration' => DAY));
      $FileTwo = new FileEngine();
      $FileTwo->init(
        array('prefix' => 'prefix_two_', 'duration' => DAY));
    ...
    public function testClearWithGroups()
    {
      ...
      $engine->init(
        array('prefix' => 'cake_test_', 'duration' => DAY, 'groups' => array('short', 'round')));
    ...
    public function testClearWithNoKeys()
    {
      $engine = new FileEngine();
      $engine->init(
        array('prefix' => 'cake_test_', 'duration' => DAY, 'groups' => array('one', 'two')));
      $key = 'cake_test_test_key';
      $engine->clear(false);
    ...
    public function testPathDoesNotExist()
    {
      ...
      Cache::config(
        'autocreate', array('engine' => 'File', 'path' => TMP . 'tests' . DS . 'autocreate'));
    ...
    {
      Cache::config(
        'repeat', array('engine' => 'File', 'groups' => array('users')));
    ...
    {
      Cache::config(
        'file_groups', array('engine' => 'File', 'duration' => 3600, 'groups' => array('group_a', 'group_b')));
      $this->assertTrue(Cache::write('test_groups', 'value', 'file_groups'));
      $this->assertEquals('value', Cache::read('test_groups', 'file_groups'));
    ...
    public function testGroupClear()
    {
      ...
      Cache::config(
        'file_groups3', array('engine' => 'File', 'duration' => 3600, 'groups' => array('group_b'), 'prefix' => 'leading_',));
    ...
    {
      Cache::config(
        'file_groups', array('engine' => 'File', 'duration' => 3600, 'prefix' => '', 'groups' => array('group_a', 'group_b')));
