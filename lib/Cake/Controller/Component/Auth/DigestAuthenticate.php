<?php
  ...
  class DigestAuthenticate extends BasicAuthenticate {
     ...
     * @var array
     */
    public $settings = array( 'fields' => array( 'username' => 'username', 'password' => 'password' ), 'userModel' => 'User', 'scope' => array(), 'recursive' => 0, 'contain' => null, 'realm' => '', 'qop' => 'auth', 'nonce' => '', 'opaque' => '', 'passwordHasher' => 'Simple', );
    ...
    public function getUser(CakeRequest $request) {
      ...
      $user = $this->_findUser(
        array( $model . '.' . $this->settings['fields']['username'] => $digest['username'] )
      );
      if( empty($user) ) {
        return false;
      }
    ...
    public function generateResponseHash($digest, $password) {
      return md5(
        $password . ':' . $digest['nonce'] . ':' . $digest['nc'] . ':' . $digest['cnonce'] . ':' . $digest['qop'] . ':' . md5(env('REQUEST_METHOD') . ':' . $digest['uri'])
      );
    }
    ...
    public function loginHeaders() {
      $options = array( 'realm' => $this->settings['realm'], 'qop' => $this->settings['qop'], 'nonce' => $this->settings['nonce'], 'opaque' => $this->settings['opaque'] );
      $opts = array();
