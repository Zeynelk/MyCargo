<?php
  ...
  class Scaffold {
     ...
     * @var array
     */
    protected $_passedVars = array( 'layout', 'name', 'viewPath', 'request' );
        ...
        if( ! empty($request->data) ) {
          ...
          if( $this->ScaffoldModel->save($request->data) ) {
            if( $this->controller->afterScaffoldSave($action) ) {
              $message = __d(
                'cake', 'The %1$s has been %2$s', Inflector::humanize($this->modelKey), $success
              );
    ...
    protected function _scaffoldDelete(CakeRequest $request) {
      if( $this->controller->beforeScaffold('delete') ) {
        ...
        $message = __d(
          'cake', 'There was an error deleting the %1$s with id: %2$s', Inflector::humanize($this->modelClass), $id
        );
        return $this->_sendMessage($message);
    ...
    protected function _scaffold(CakeRequest $request) {
      ...
      if( isset($db) ) {
        if( empty($this->scaffoldActions) ) {
          ...
          $this->scaffoldActions = array( $scaffoldPrefix . '_index', $scaffoldPrefix . '_list', $scaffoldPrefix . '_view', $scaffoldPrefix . '_add', $scaffoldPrefix . '_create', $scaffoldPrefix . '_edit', $scaffoldPrefix . '_update', $scaffoldPrefix . '_delete' );
        }
        ...
        if( in_array($request->params['action'], $this->scaffoldActions) ) {
          ...
          throw new MissingActionException(
            array( 'controller' => $this->controller->name, 'action' => $request->action )
          );
        }
      } else {
