<?php
  ...
  class User extends UsersAppModel {
     ...
     */
    public $filterArgs = array( 'username' => array( 'type' => 'like' ), 'email' => array( 'type' => 'value' ) );
    ...
    protected function _setupBehaviors() {
      ...
      if( CakePlugin::loaded('Utils') && class_exists('SluggableBehavior') && Configure::read('Users.disableSlugs') !== true ) {
        $this->actsAs['Utils.Sluggable'] = array( 'label' => 'username', 'method' => 'multibyteSlug' );
      }
    }
    ...
    protected function _setupValidation() {
      $this->validatePasswordChange = array( 'new_password' => $this->validate['password'], 'confirm_password' => array( 'required' => array( 'rule' => array( 'compareFields', 'new_password', 'confirm_password' ), 'required' => true, 'message' => __d('users', 'The passwords are not equal.') ) ), 'old_password' => array( 'to_short' => array( 'rule' => 'validateOldPassword', 'required' => true, 'message' => __d('users', 'Invalid password.') ) ) );
    }
    ...
    public function checkEmailVerfificationToken($token = null) {
      $result = $this->find(
        'first', array( 'contain' => array(), 'conditions' => array( $this->alias . '.email_verified' => 0, $this->alias . '.email_token' => $token ), 'fields' => array( 'id', 'email', 'email_token_expires', 'role' ) )
      );
    ...
    public function verifyEmail($token = null) {
      ...
      $user = $this->save(
        $user, array( 'validate' => false, 'callbacks' => false )
      );
      $this->data = $user;
      return $user;
    }
    ...
    public function passwordReset($postData = array()) {
      $user = $this->find(
        'first', array( 'contain' => array(), 'conditions' => array( $this->alias . '.active' => 1, $this->alias . '.email' => $postData[$this->alias]['email'] ) )
      );
    ...
    public function checkPasswordToken($token = null) {
      $user = $this->find(
        'first', array( 'contain' => array(), 'conditions' => array( $this->alias . '.active' => 1, $this->alias . '.password_token' => $token, $this->alias . '.email_token_expires >=' => date('Y-m-d H:i:s') ) )
      );
      if( empty($user) ) {
        return false;
      }
    ...
    public function setUpResetPasswordValidationRules() {
      return array( 'new_password' => $this->validate['password'], 'confirm_password' => array( 'required' => array( 'rule' => array( 'compareFields', 'new_password', 'confirm_password' ), 'message' => __d('users', 'The passwords are not equal.') ) ) );
    }
    ...
    public function resetPassword($postData = array()) {
      ...
      if( $this->validates() ) {
        ...
        $result = $this->save(
          $this->data, array( 'validate' => false, 'callbacks' => false )
        );
      }
    ...
    public function changePassword($postData = array()) {
      ...
      if( $this->validates() ) {
        $this->data[$this->alias]['password'] = $this->hash($this->data[$this->alias]['new_password'], null, true);
        $this->save(
          $postData, array( 'validate' => false, 'callbacks' => false )
        );
        return true;
      }
      return false;
    ...
    public function compareFields($field1, $field2) {
      ...
      if( isset($this->data[$this->alias][$field1]) && isset($this->data[$this->alias][$field2]) && $this->data[$this->alias][$field1] == $this->data[$this->alias][$field2] ) {
        return true;
      }
      return false;
    }
    ...
    public function view($slug = null, $field = 'slug') {
      $user = $this->find(
        'first', array( 'contain' => array(), 'conditions' => array( 'OR' => array( $this->alias . '.' . $field => $slug, $this->alias . '.' . $this->primaryKey => $slug ), $this->alias . '.active' => 1, $this->alias . '.email_verified' => 1 ) )
      );
    ...
    public function findByEmail($email = null) {
      return $this->find(
        'first', array( 'contain' => array(), 'conditions' => array( $this->alias . '.email' => $email, ) )
      );
    }
    ...
    public function register($postData = array(), $options = array()) {
      $Event = new CakeEvent(
        'Users.Model.User.beforeRegister', $this, array( 'data' => $postData, 'options' => $options )
      );
      ...
      if( $this->validates() ) {
        ...
        $Event = new CakeEvent(
          'Users.Model.User.afterRegister', $this, array( 'data' => $this->data, 'options' => $options )
        );
    ...
    protected function _findSearch($state, $query, $results = array()) {
      ...
      if( $state == 'before' ) {
        $this->Behaviors->load(
          'Containable', array( 'autoFields' => false )
        );
        $results = $query;
        ...
        switch( $by ) {
          ...
          case 'any':
            $results['conditions'] = Hash::merge(
              $query['conditions'], array( 'OR' => array( array( $this->alias . '.username LIKE' => $like ), array( $this->alias . '.email LIKE' => $like ) ) )
            );
            break;
          case '' :
    ...
    public function getUserForEditing($userId = null, $options = array()) {
      $defaults = array( 'contain' => array(), 'conditions' => array( $this->alias . '.id' => $userId ) );
    ...
    protected function _removeExpiredRegistrations() {
      $this->deleteAll(
        array( $this->alias . '.email_verified' => 0, $this->alias . '.email_token_expires <' => date('Y-m-d H:i:s') )
      );
    }
