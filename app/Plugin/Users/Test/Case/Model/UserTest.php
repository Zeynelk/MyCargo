<?php
  ...
  class UserTestCase extends CakeTestCase
  {
    ...
    {
      $this->User->data['User'] = array('email' => 'test@email.com');
    ...
    {
      $data = array('User' => array('id' => 1, 'new_password' => '', 'confirm_password' => 'dsgdsgsdg'));
      $this->assertFalse($this->User->resetPassword($data));

      $data = array('User' => array('id' => 1, 'new_password' => '', 'confirm_password' => ''));
      $this->assertFalse($this->User->resetPassword($data));

      $data = array('User' => array('id' => 1, 'new_password' => 'newpassword', 'confirm_password' => 'newpassword'));
    ...
    {
      $data = array('User' => array('id' => 1, 'email' => 'somethingwrong in here!'));
      $this->assertFalse($this->User->passwordReset($data));

      $this->User->id = '1';
      $this->User->saveField('email_token_expires', date('Y-m-d H:i:s', strtotime('+1 year')));
      $data = array('User' => array('id' => 1, 'email' => 'adminuser@cakedc.com'));
    ...
    public function testValidateOldPassword()
    {
      ...
      $this->User->data = array('User' => array('id' => '1', 'password'));
    ...
    public function testRegister()
    {
      ...
      $postData = array('User' => array('username' => '#236236326sdg!!!.s#invalid', 'email' => 'invalid', 'password' => 'password', 'temppassword' => 'wrong', 'tos' => 0));
      $result = $this->User->register($postData);
      $this->assertFalse($result);
      $this->assertEquals(
        array_keys($this->User->invalidFields()), array('username', 'email', 'temppassword', 'tos'));

      $postData = array('User' => array('username' => 'validusername', 'email' => 'test@test.com', 'password' => '12345', 'temppassword' => '12345', 'tos' => 1));
      $result = $this->User->register($postData);
      $this->assertFalse($result);
      $this->assertEquals(
        array_keys($this->User->invalidFields()), array('password'));

      $postData = array('User' => array('username' => 'imanewuser', 'email' => 'foo@bar.com', 'password' => 'password', 'temppassword' => 'password', 'tos' => 1));
    ...
    public function testChangePassword()
    {
      ...
      $result = $this->User->changePassword($postData);
      $this->assertTrue($result);
      $ressult = $this->User->find(
        'first', array('recursive' => -1, 'conditions' => array('User.id' => 1)));
    ...
    {
      $this->User->data = array('User' => array('field1' => 'foo', 'field2' => 'bar'));
      $this->assertFalse($this->User->compareFields('field1', 'field2'));

      $this->User->data = array('User' => array('field1' => 'foo', 'field2' => 'foo'));
    ...
    public function testResendVerification()
    {
      ...
      $postData = array('User' => array('email' => 'doesnotexist!'));
      $this->assertFalse($this->User->resendVerification($postData));

      $postData = array('User' => array('email' => 'adminuser@cakedc.com'));
      $this->assertFalse($this->User->resendVerification($postData));

      $postData = array('User' => array('email' => 'oidtest2@testuser.com'));
    ...
    {
      $postData = array('User' => array('username' => 'newusername', 'email' => 'newusername@newusername.com', 'password' => 'password', 'temppassword' => 'password', 'tos' => 1));
    ...
    public function testDisableSlugs()
    {
      ...
      $this->User->create();
      $this->User->save(
        array('username' => 'foo2'), array('validate' => false));
      ...
      $this->User->create();
      $this->User->save(
        array('username' => 'bar2'), array('validate' => false));
      $result = $this->User->read(null, $this->User->id);
