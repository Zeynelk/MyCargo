<?php
  ...
  class LookupableBehavior extends ModelBehavior {
    ...
    public function findExistingRecord(Model $Model, $modelClass, $name) {
      return $Model->{$modelClass}->find(
        'first', array( 'contain' => array(), 'conditions' => array( $modelClass . '.' . $Model->{$modelClass}->displayField => $name ) )
      );
    }
    ...
    public function saveLookupRecord(Model $Model, $modelClass, $data) {
      ...
      return $Model->{$modelClass}->save(
        array( $modelClass => $data ), array( 'validate' => false, 'callbacks' => true )
      );
    }
    ...
    public function afterSave(Model $Model, $created) {
      ...
      if( $resave ) {
        $result = $Model->save(
          $Model->data, array( 'validate' => false, 'callbacks' => false )
        );
        $Model->data = $result;
      }
    }
