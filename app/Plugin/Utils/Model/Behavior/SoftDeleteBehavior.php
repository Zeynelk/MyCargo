<?php
  ...
  class SoftDeleteBehavior extends ModelBehavior {
    ...
    public function delete($model, $id) {
      ...
      $record = $model->find(
        'first', array( 'fields' => $model->primaryKey, 'conditions' => array( $model->primaryKey => $id ), 'recursive' => -1 )
      );
    ...
    public function purgeDeletedCount($model, $expiration = '-90 days') {
      $this->softDelete($model, false);
      return $model->find(
        'count', array( 'conditions' => $this->_purgeDeletedConditions($model, $expiration), 'recursive' => -1, 'contain' => array() )
      );
    }
    ...
    public function purgeDeleted($model, $expiration = '-90 days') {
      $this->softDelete($model, false);
      $records = $model->find(
        'all', array( 'conditions' => $this->_purgeDeletedConditions($model, $expiration), 'fields' => array( $model->primaryKey ), 'recursive' => -1 )
      );
      if( $records ) {
