<?php
  ...
  class KeyvalueBehavior extends ModelBehavior {
    ...
    public function getSection($Model, $foreignKey = null, $section = null) {
      $Model->recursive = -1;
      $results = $Model->find(
        'all', array( 'conditions' => array( $this->settings[$model->alias]['foreignKey'] => $foreignKey ) ), array( 'fields' => array( 'field', 'value' ) )
      );
    ...
    public function saveSection($Model, $foreignKey = null, $data = null, $section = null) {
      foreach( $data as $model => $details ) {
        foreach( $details as $key => $value ) {
          ...
          $Model->recursive = -1;
          $tmp = $this->find(
            'first', array( 'conditions' => array( $this->settings[$model->alias]['foreignKey'] => $foreignKey, 'field' => $section . '.' . $key ), 'fields' => array( 'id' ) )
          );
