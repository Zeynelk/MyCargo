<?php
  ...
  class ListBehavior extends ModelBehavior {
     ...
     * @var array
     */
    protected $_defaults = array( 'positionColumn' => 'position', 'scope' => '', 'validate' => false, 'addToTop' => false, 'callbacks' => false );
    ...
    public function fixListOrder($model) {
      extract($this->settings[$model->alias]);
      $data = $model->find(
        'all', array( 'conditions' => $this->__scopeCondition($model), 'order' => array( $model->alias . '.' . $positionColumn => 'asc' ), 'recursive' => -1 )
      );
      $position = 1;
      foreach( $data as $row ) {
        $model->id = $row[$model->alias][$model->primaryKey];
        $model->saveField(
          $positionColumn, $position, array( 'validate' => $validate, 'callbacks' => $callbacks )
        );
        $position += 1;
      }
    }
    ...
    protected function _incrementPosition($model) {
      ...
      return $model->save(
        null, array( 'validate' => $validate, 'callbacks' => $callbacks )
      );
    }
    ...
    private function __decrementPositionsOnHigherItems(Model $model, $position) {
      ...
      return $model->updateAll(
        array( $model->alias . '.' . $positionColumn => $model->alias . '.' . $positionColumn . '-1' ), array( $this->__scopeCondition($model), $model->alias . '.' . $positionColumn . ' <= ' => $position )
      );
    }
    ...
    private function __decrementPositionsOnLowerItems($model) {
      ...
      $positionColumn = $this->settings[$model->alias]['positionColumn'];
      return $model->updateAll(
        array( $model->alias . '.' . $positionColumn => $model->alias . '.' . $positionColumn . ' - 1' ), array( $this->__scopeCondition($model), $model->alias . '.' . $positionColumn . ' > ' => $model->data[$model->alias][$positionColumn] )
      );
    }
    ...
    private function __incrementPositionsOnAllItems($model) {
      extract($this->settings[$model->alias]);
      return $model->updateAll(
        array( $model->alias . '.' . $positionColumn => $model->alias . '.' . $positionColumn . '+1' ), array( $this->__scopeCondition($model) )
      );
