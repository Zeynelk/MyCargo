<?php
  ...
  class BtreeBehavior extends ModelBehavior {
     ...
     * @var array
     */
    protected $_defaults = array( 'parent' => 'parent_id', 'left' => 'lft', 'right' => 'rght', 'scope' => '1 = 1', 'type' => 'nested', '__parentChange' => false, 'recursive' => -1, 'block_size' => 32768 //block size of 32k allows for up to 64k trees (MAX_INT / 2*block_size)
    );
    ...
    public function beforeDelete(Model $Model, $cascade = true) {
      extract($this->settings[$Model->alias]);
      $data = $Model->find(
        'first', array( 'conditions' => array( $Model->escapeField($Model->primaryKey) => $Model->id ), 'fields' => array( $Model->escapeField($left), $Model->escapeField($right) ), 'recursive' => -1 )
      );
      if( $data ) {
    ...
    public function beforeSave(Model $Model) {
      ...
      if( ! $Model->id || ! $Model->exists() ) {
        if( array_key_exists($parent, $Model->data[$Model->alias]) && $Model->data[$Model->alias][$parent] ) {
          $parentNode = $Model->find(
            'first', array( 'conditions' => array( $scope, $Model->escapeField() => $Model->data[$Model->alias][$parent] ), 'fields' => array( $Model->primaryKey, $right ), 'recursive' => $recursive )
          );
          if( ! $parentNode ) {
            return false;
          }
        ...
        if( ! $Model->data[$Model->alias][$parent] ) {
          ...
          $values = $Model->find(
            'first', array( 'conditions' => array( $scope, $Model->escapeField() => $Model->id ), 'fields' => array( $Model->primaryKey, $parent, $left, $right ), 'recursive' => $recursive )
          );
          ...
          $parentNode = $Model->find(
            'first', array( 'conditions' => array( $scope, $Model->escapeField() => $Model->data[$Model->alias][$parent] ), 'fields' => array( $Model->primaryKey, $left, $right ), 'recursive' => $recursive )
          );
          if( ! $parentNode ) {
            return false;
          }
    ...
    public function children(Model $Model, $id = null, $direct = false, $fields = null, $order = null, $limit = null, $page = 1, $recursive = null) {
      ...
      if( ! $id ) {
        ...
        $result = array_values(
          (array)$Model->find(
            'first', array( 'conditions' => array( $scope, $Model->escapeField() => $id ), 'fields' => array( $left, $right ), 'recursive' => $recursive )
          )
        );
    ...
    public function getPath(Model $Model, $id = null, $fields = null, $recursive = null) {
      ...
      $item = $result[0];
      $results = $Model->find(
        'all', array( 'conditions' => array( $scope, $Model->escapeField($left) . ' <=' => $item[$left], $Model->escapeField($right) . ' >=' => $item[$right] ), 'fields' => $fields, 'order' => array( $Model->escapeField($left) => 'asc' ), 'recursive' => $recursive )
      );
      return $results;
    }
    ...
    public function moveDown(Model $Model, $id = null, $number = 1) {
      ...
      extract($this->settings[$Model->alias]);
      list($node) = array_values(
        $Model->find(
          'first', array( 'conditions' => array( $scope, $Model->escapeField() => $id ), 'fields' => array( $Model->primaryKey, $left, $right, $parent ), 'recursive' => $recursive )
        )
      );
      if( $node[$parent] ) {
        list($parentNode) = array_values(
          $Model->find(
            'first', array( 'conditions' => array( $scope, $Model->escapeField() => $node[$parent] ), 'fields' => array( $Model->primaryKey, $left, $right ), 'recursive' => $recursive )
          )
        );
      ...
      $nextNode = $Model->find(
        'first', array( 'conditions' => array( $scope, $Model->escapeField($left) => ($node[$right] + 1) ), 'fields' => array( $Model->primaryKey, $left, $right ), 'recursive' => $recursive )
      );
      if( $nextNode ) {
    ...
    public function moveUp(Model $Model, $id = null, $number = 1) {
      ...
      $previousNode = $Model->find(
        'first', array( 'conditions' => array( $scope, $Model->escapeField($right) => ($node[$left] - 1) ), 'fields' => array( $Model->primaryKey, $left, $right ), 'recursive' => $recursive )
      );
    ...
    public function recover(Model $Model, $mode = 'parent', $missingParentAction = null) {
      ...
      if( $mode === 'parent' ) {
        $Model->bindModel(
          array( 'belongsTo' => array( 'VerifyParent' => array( 'className' => $Model->name, 'foreignKey' => $parent, 'fields' => array( $Model->primaryKey, $left, $right, $parent ), ) ) )
        );
        $missingParents = $Model->find(
          'list', array( 'recursive' => 0, 'conditions' => array( $scope, array( 'NOT' => array( $Model->escapeField($parent) => null ), $Model->VerifyParent->escapeField() => null ) ) )
        );
    ...
    public function removeFromTree(Model $Model, $id = null, $delete = false) {
      ...
      if( $delete ) {
        $Model->updateAll(
          array( $Model->escapeField($left) => 0, $Model->escapeField($right) => 0, $Model->escapeField($parent) => null ), array( $Model->escapeField() => $id )
        );
        return $Model->delete($id);
      } else {
        /*
        ...
        $Model->id = $id;
        return $Model->save(
          array( $left => $edge, $right => $edge + 1, $parent => null ), array( 'callbacks' => false, 'validate' => false )
        );
      }
    }
    ...
    function __verifyRelations(Model $Model) {
      ...
      $Model->bindModel(
        array( 'belongsTo' => array( 'VerifyParent' => array( 'className' => $Model->alias, 'foreignKey' => $parent, 'fields' => array( $Model->primaryKey, $left, $right, $parent ) ) ) )
      );
      $nodePage = 0;
      $nodes = $Model->find(
        'all', array( 'conditions' => am($scope), 'fields' => array( $Model->primaryKey, $left, $right, $parent, 'VerifyParent.' . $Model->primaryKey, 'VerifyParent.' . $left, 'VerifyParent.' . $right ), 'limit' => 1024, 'page' => $nodePage++, 'order' => $Model->escapeField($Model->primaryKey), 'recursive' => 1 )
      );
    ...
    function __verifyContinuity(Model $Model) {
      ...
      foreach( $nodes as $node ) {
        ...
        for( $i = $min; $i <= $max; $i++ ) {
          $count = $Model->find(
            'count', array( 'conditions' => array( $scope, 'OR' => array( $Model->escapeField($left) => $i, $Model->escapeField($right) => $i ) ) )
          );
          if( $count != 1 ) {
            if( $count == 0 ) {
    ...
    protected function _setParent(Model $Model, $parentId = null, $created = false) {
      ...
      if( empty($parentId) ) {
        ...
        $values = $Model->find(
          'first', array( 'conditions' => array( $scope, $Model->escapeField() => $parentId ), 'fields' => array( $Model->primaryKey, $left, $right ), 'recursive' => $recursive )
        );
    ...
    protected function _getPartition(Model $Model, $scope, $recursive = -1, $created = false) {
      ...
      $edge_pairs = $Model->find(
        'list', array( 'conditions' => array( $scope, 'parent_id' => null ), 'fields' => array( $left, $right ), 'order' => "lft ASC", 'recursive' => $recursive )
      );
    ...
    protected function _getMax(Model $Model, $scope, $right, $recursive = -1, $created = false) {
      ...
      list($edge) = array_values(
        $Model->find(
          'first', array( 'conditions' => $scope, 'fields' => $db->calculate($Model, 'max', array( $name, $right )), 'recursive' => $recursive, 'callbacks' => false )
        )
      );
    ...
    protected function _getMin(Model $Model, $scope, $left, $recursive = -1) {
      ...
      list($edge) = array_values(
        $Model->find(
          'first', array( 'conditions' => $scope, 'fields' => $db->calculate($Model, 'min', array( $name, $left )), 'recursive' => $recursive, 'callbacks' => false )
        )
      );
