<?php
  ...
  class TinySluggableBehavior extends ModelBehavior {
    ...
    private function __getNextSlug(Model $Model) {
      $new = '';
      $prev = $Model->find(
        'first', array( 'contain' => array(), 'fields' => array( "{$Model->alias}.{$Model->tinySlug}", "{$Model->alias}.created" ), 'order' => "{$Model->alias}.{$this->settings[$Model->alias]['orderField']} DESC" )
      );
      ...
      if( empty($prev) ) {
        ...
        do {
          if( $attempts == 1 ) {
            $maxAttempts = $Model->find(
              'count', array( 'conditions' => array( $Model->alias . '.created' => $prev[$Model->alias]['created'] ) )
            );
          }
          $new = $this->__toShort($Model, $this->__toDecimal($Model, $new) + 1);
          $existing = $Model->find(
            'count', array( 'conditions' => array( $Model->alias . '.' . $Model->tinySlug => $new ) )
          );
          $attempts++;
