<?php
  ...
  class EmailErrorHandler extends ErrorHandler {
    ...
    public static function handlerSettings() {
      $defaults = array( 'caching' => true, 'receiver' => null, 'emailNotifications' => false, 'duration' => ini_get('max_execution_time'), 'codes' => array(), 'logLevels' => array() );
    ...
    public static function setCacheConfig($duration) {
      Cache::config(
        'error_handler', array( 'engine' => 'File', 'duration' => '+' . $duration . ' seconds' )
      );
    }
    ...
    public static function getEmailInstance() {
      if( empty(self::$Email) ) {
        $Email = new CakeEmail();
        $Email->subject(__d('utils', 'Error notification from %s', env('HTTP_HOST')))->from(array( 'error@' . env('HTTP_HOST') => 'Error Handler' ))->to(Configure::read('ErrorHandler.receiver'))->emailFormat('both')->template('Utils.error_notification');
        self::$Email = $Email;
      }
      return self::$Email;
    }
