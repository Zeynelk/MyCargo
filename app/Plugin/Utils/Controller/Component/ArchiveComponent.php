  ...
  class ArchiveComponent extends Component {
    ...
    protected function _buildArchiveConditions($dateParams) {
      ...
      if( strtotime($startDate) > time() ) {
        $this->cakeError(
          'error', array( 'name' => sprintf(__d('utils', 'No %s found for that date range'), Inflector::humanize(Inflector::pluralize($this->modelName))), 'message' => $this->controller->here, 'code' => 404, )
        );
        $this->_stop();
      }
